  ...
  if (req.method === 'GET') { // Get all unhidden tasks or collections for the logged in user
    ...
    if (collection === "true") {
      ...
      if (!name || !description) {
        ...
        return;
      } else if (user.stats.collections >= 100) { // limit
      ...
      //const limit = await db.collection('tasks').countDocuments({ owner: new ObjectId(user.id) }); // REPLACE with task count from user API
      if (!name || !description) {
        res.status(422).json({ message: "Invalid data" });
        return;
      } else if (name.trim().length > 55 || description.trim().length > 500) {
        res.status(422).json({ message: "Length of title and description must not exceed 55 and 500 characters respectively." });
        return;
      } else if (user.stats.tasks >= 10000) { // limit
