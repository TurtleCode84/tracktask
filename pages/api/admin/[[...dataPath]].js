  ...
  if (dataPath[0] === "tasks") {
    ...
    if (req.method === 'GET') { // Returns a task or array of tasks, but only if reported
      ...
      if (dataPath[1] && !ObjectId.isValid(dataPath[1])) {
        ...
        var reportedTaskIds = [];
        reportedTasks.forEach(task => reportedTaskIds.push(new ObjectId(task.reported)));
      ...
      // Return data
      res.json({...data, debug: {"reportedTaskIds": reportedTaskIds, "reportedTasks": reportedTasks.length}});
    ...
    if (req.method === 'GET') { // Returns a collection or array of collections, but only if reported
      ...
      if (dataPath[1] && !ObjectId.isValid(dataPath[1])) {
        ...
        reportedCollections.forEach(collection => reportedCollectionIds.push(new ObjectId(collection.reported)));
      ...
      // Return data
      res.json({...data, debug: {"reportedCollectionIds": reportedCollectionIds, "reportedCollections": reportedCollections.length}});
