...
async function adminUsersRoute(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (sort === "joined") {
      try {
        const getUsers = await db.collection("users").find().limit(parseInt(count))/*.projection({ _id: 1, username: 1, 'history.joined': 1 }).limit(count).sort({ 'history.joined': -1 })*/.toArray();
        if (getUsers) {
          res.json(getUsers);
        } else {
