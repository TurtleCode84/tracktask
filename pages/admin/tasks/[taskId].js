...
export default function TaskAdmin() {
  ...
  useEffect(() => {
    ...
    const collectionTags = filteredCollections?.map((item, index) =>
    ...
    <Layout>
      ...
      {task ?
        ...
        <details id="raw">
          ...
          {taskError ? <pre>{JSON.stringify(taskError, null, 2)}</pre> : <pre>{JSON.stringify(task, null, 2)}</pre>}<pre>{JSON.stringify(collectionTags, null, 2)}</pre>
        </details><br/>
