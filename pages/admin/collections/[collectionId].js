...
export default function Collection() {
  ...
  const relTaskList = collection?.tasks?.filter(task => task.completion.completed === 0).sort((a, b) => a.dueDate < b.dueDate || b.dueDate === 0 ? 1 : 0).reverse().sort((a, b) => a.priority ? -1 : 1).map((task) =>
    <Task task={task} key={task._id} admin={true}/>
  );
  const comTaskList = collection?.tasks?.filter(task => task.completion.completed > 0).sort((a, b) => a.dueDate < b.dueDate || b.dueDate === 0 ? 1 : 0).reverse().sort((a, b) => a.priority ? -1 : 1).map((task) =>
    <Task task={task} key={task._id} admin={true}/>
  );
