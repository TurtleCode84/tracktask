...
export default function Settings() {
  ...
  return (
    <Layout>
      ...
      <p>Profile picture: <span style={{ verticalAlign: "middle", borderRadius: "100%", overflow: "hidden" }}><Image src={user.profilePicture ? user.profilePicture : "/default-pfp.jpg" } width={32} height={32} alt=""/></span> ({user.profilePicture ? <a href={user.profilePicture} target="_blank" rel="noreferrer">link</a> : 'default'})</p>
      <h3>History</h3>
      ...
      <p style={{ fontStyle: "italic" }} title={user.lastEdit?.timestamp > 0 ? moment.unix(user.lastEdit?.timestamp).format("dddd, MMMM Do YYYY, h:mm:ss a") : 'Never'}>Last edited {user.lastEdit?.timestamp > 0 ? moment.unix(user.lastEdit?.timestamp).fromNow() : 'never'}{user.lastEdit?.by && user.lastEdit?.by !== user.id && ' by an administrator'}</p>
    </Layout>    
  );
}
