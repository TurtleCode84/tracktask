...
export default function Dashboard() {
  ...
  const taskList = sortedTasks?.map((task) =>
    <li key={task._id}>
      {task.priority ? <>&#10071;</> : null}{task.name} - {task.description} (due <DueDate timestamp={task.dueDate}/>{task.dueDate !== 0 ? <>, on {moment.unix(task.dueDate).local().format("dddd, MMMM Do YYYY, h:mm:ss a")}</> : null})
    </li>
  );
