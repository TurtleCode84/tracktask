...
export default function Collection({ user, collection, key, admin }) {
  ...
  return (
    <li key={key} className="list-hover" style={{ margin: "0.5em", background: "var(--element-background)", padding: "8px", borderWidth: "2px", borderStyle: "solid", borderColor: "var(--border-color)", borderRadius: "10px", width: "100%" }} onClick={() => router.push(`/${admin === true ? 'admin/' : ''}collections/${collection._id}`)}>
      ...
      <div style={{ marginLeft: "22px", marginTop: "3px", color: "#555" }}>{collection.description.length > 50 ? <>{collection.description.slice(0,50).trim()}...</> : collection.description}</div>
    </li>
  );
}
