...
export default function Layout({ children }) {
  ...
  if (process.env.ADVISORY.split(',')[0] !== "default") {
    advisoryColor = process.env.ADVISORY.split(',')[0];
  }
  return (
    <>
      <Head>
      ...
      {process.env.ADVISORY && <nav style={{backgroundColor: advisoryColor, textAlign: "center", height:40 + "px", fontWeight: 600, marginTop: -20 + "px", paddingBottom: 5 + "px", minHeight: "min-content"}}><p style={{paddingTop: 13 + "px"}}>{process.env.ADVISORY?.split(',')[1]}</p></nav>}
      <Header />
