...
export default function Task({ task, key }) {
  ...
  const collectionTags = task.collections?.map((item, index) =>
    <span key={index} style={{backgroundColor: stringToColor(item.name), padding: "0.5px 4px", borderStyle: "solid", borderWidth: "2px", borderColor: "var(--inset-border-color)", borderRadius: "7px", color: "#111", marginTop: "5px", marginLeft: "20px", marginRight: "-15px", display: "inline-block", filter: "grayscale(0.4) brightness(1.5)" }} onClick={(e) => {e.stopPropagation(); router.push(`/collections/${item._id}`);}}>{item.name}</span>
  );
  return (
