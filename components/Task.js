...
export default function Task({ task, key }) {
  ...
  const collectionTags = task.collections?.map((item, index) =>
    <span key={index} className="list-inset" style={{backgroundColor: stringToColor(item.name), paddingTop: "0.5px", paddingLeft: "4px", paddingBottom: "0.5px", paddingRight: "4px", borderColor: "var(--border-color)", borderStyle: "solid", borderWidth: "2px", borderRadius: "7px", color: "#111", marginTop: "5px", marginLeft: "20px", marginRight: "-15px", display: "inline-block", filter: "grayscale(0.4) brightness(1.5)" }}>{item.name}</span>
  );
  return (
