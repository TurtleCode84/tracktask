...
export default function Task({ task, key }) {
  ...
  const collectionTags = task.collections?.map((item, index) =>
    <span key={index} style={{backgroundColor: stringToColor(item), paddingTop: "0.5px", paddingLeft: 4 + "px", paddingBottom: "0.5px", paddingRight: 4 + "px", borderColor: "darkgray", borderStyle: "solid", borderWidth: 2 + "px", borderRadius: 10 + "px", color: "black", marginTop: 5 + "px", marginLeft: 20 + "px", marginRight: -15 + "px", display: "inline-block" }}>{item}</span>
  );
  return (
